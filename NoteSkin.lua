...
function NSKIN.Load()
	...
	local Reverse = sPlayer and string.find(GAMESTATE:GetPlayerState(sPlayer):GetPlayerOptionsString("ModsLevel_Preferred"):lower(), "reverse")

    local Button = NSKIN.ButtonRedir[sButton] or sButton
    local Element = NSKIN.ElementRedir[sElement] or sElement

    if (string.find(Element, "Active") or string.find(Element, "Inactive")) and
        (not string.find(Element, "Head") and not string.find(Element, "Roll")) then
        Button = sButton
	...
	if type(Actor) == "function" then
		...
		Actor = Def.Sprite {
			...
			HoldUpdateCommand = function(self) 
                self:customtexturerect(0, -(self:GetHoldLength() / self:GetHeight()), 1, 0)
            end
		}
	end
